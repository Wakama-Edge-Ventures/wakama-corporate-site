...
import {ArrowRight, BriefcaseBusiness, CheckCircle2, ClipboardCheck, Sparkles} from "lucide-react";
...
export function PilotHero() {
  const t = useTranslations("pilotPage.hero");
  const locale = useLocale();
  const shouldReduceMotion = useReducedMotion();
  const proofs = t.raw("proofs") as string[];
  const visualItems = t.raw("visualItems") as Array<{label: string; value: string}>;
  const visualHighlights = t.raw("visualHighlights") as Array<{label: string; value: string}>;
      ...
      <Container className="relative">
        <div className="grid items-center gap-9 lg:grid-cols-[minmax(0,1fr)_minmax(320px,26.5rem)] lg:gap-16 xl:gap-20">
          ...
          >
            ...
            <p className="mt-5 max-w-2xl text-sm leading-6 text-white/62 sm:text-[0.96rem]">
              {t("positioning")}
            </p>
            ...
            <div className="mt-7 flex flex-col gap-3 sm:flex-row">
              ...
              <Button
                href={`/${locale}/pilot#pilot-deliverables`}
                variant="secondary"
                size="md"
          ...
          >
            ...
            <div className="relative">
              <div className="flex items-start justify-between gap-4">
                ...
                <span className="rounded-full border border-mintCta/24 bg-mintCta/10 px-3 py-1 font-mono text-[10px] font-semibold uppercase tracking-[0.16em] text-mintCta sm:text-xs">
                  {t("visualStatus")}
                </span>
              </div>

              <div className="mt-4 space-y-2">
                {visualItems.map(({label, value}, index) => {
                  const Icon = [BriefcaseBusiness, ClipboardCheck, CheckCircle2][index] ?? CheckCircle2;
                  const tone = ["text-cyanLogo", "text-mintCta", "text-orangeAccent"][index] ?? "text-cyanLogo";

                  return (
                    <div key={label} className="rounded-lg border border-white/10 bg-backgroundDeep/62 px-3 py-2.5">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-mono text-[10px] uppercase tracking-[0.16em] text-white/42 sm:text-[11px]">
                            {label}
                          </p>
                          <p className="mt-1 text-[13.5px] font-normal leading-5 text-white/82 sm:text-sm">
                            {value}
                          </p>
                        </div>
                      </div>
